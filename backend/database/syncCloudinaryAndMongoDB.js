...
const fetchAllDomainsFromCloudinaryAndAddToDatabase = async () => {
  const fetchAllDomainsFromCloudinaryAndAddToDatabaseTransaction =
  ...
  try {
    ...
    const domainsMissingInDatabase = domains.folders.filter((domain) => {
      ...
      return !domainsInDatabase.find(
        (dbDomain) => dbDomain.name === domain.name
      );
    });
    ...
    if (extraDomainsInDatabase.length > 0) {
      await Domain.deleteMany({
        _id: { $in: extraDomainsInDatabase.map((domain) => domain._id) },
      });
    }
...
const fetchAllSubdomainsFromCloudinaryAndAddToDatabase = async () => {
  const fetchAllSubdomainsFromCloudinaryAndAddToDatabaseTransaction =
    Sentry.startTransaction({
  ...
  try {
    ...
    const subdomains = {};
    await Promise.all(
      domains.folders.map(async (domain) => {
        subdomains[domain.name] = await cloudinary.api
          .sub_folders(`noteups/${domain.name}`)
          .then((subdomains) => subdomains.folders);
      })
    );
    const subdomainsInDatabase = await Subdomain.find({});

    // Check if there are subdomains missing in the database
    const subdomainsMissingInDatabase = Object.keys(subdomains).map(
      (domain) => {
        return subdomains[domain].filter((subdomain) => {
          finalSubdomains.push(subdomain.name);
          return !subdomainsInDatabase.find(
            (dbSubdomain) => dbSubdomain.name === subdomain.name
          );
        });
      }
    );
    ...
    const extraSubdomainsInDatabase = subdomainsInDatabase.filter(
      (dbSubdomain) => {
        return !finalSubdomains.find(
          (subdomain) => subdomain === dbSubdomain.name
        );
      }
    );
    ...
    if (extraSubdomainsInDatabase.length > 0) {
      await Subdomain.deleteMany({
        _id: {
          $in: extraSubdomainsInDatabase.map((subdomain) => subdomain._id),
        },
      });
    }
...
const fetchAllSubjectsFromCloudinaryAndAddToDatabase = async () => {
  const fetchAllSubjectsFromCloudinaryAndAddToDatabaseTransaction =
    Sentry.startTransaction({
  ...
  try {
    ...
    await Promise.all(
      Object.keys(subdomains).map(async (domain) => {
        subjects[domain] = await Promise.all(
          subdomains[domain].map(async (subdomain) => {
            return await cloudinary.api
              .sub_folders(`noteups/${domain}/${subdomain.name}`)
              .then((subjects) => subjects.folders);
          })
        );
      })
    );
    ...
    const subjectsMissingInDatabase = Object.keys(subjects).map((domain) => {
      return subjects[domain].map((subjects) => {
        return subjects.map((subject) => {
          ...
          if (
            subjectsInDatabase.find(
              (dbSubject) => dbSubject.name === subject.name
            )
          ) {
            return false;
          } else {
    ...
    if (subjectsMissingInDatabase.length > 0) {
      const subjectsToAdd = subjectsMissingInDatabase.map(
        (allDomainSubjects) => {
            ...
            return allSubdomainSubjects.map((subject) => {
              if (subject != false) {
                return {
                  _id: new mongoose.Types.ObjectId(),
                  name: subject.name,
                  subdomain: subject.path.split("/")[2],
                  domain: subject.path.split("/")[1],
                  createdAt: new Date(),
                  updatedAt: new Date(),
                };
              } else {
    ...
    if (extraSubjectsInDatabase.length > 0) {
      await Subject.deleteMany({
        _id: { $in: extraSubjectsInDatabase.map((subject) => subject._id) },
      });
    }
