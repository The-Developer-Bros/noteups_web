...
const Upload = () => {
  ...
  const uploadImage = async (base64EncodedImage) => {
    ...
    try {
      await fetch(`${process.env.REACT_APP_BACKEND_URL}/productApi/upload`, {
        method: "POST",
        body: JSON.stringify({
  ...
  const getDomains = async () => {
    try {
      ...
      const domainJson = await backendClient.get("/productApi/domains");
  ...
  const getSubjects = async (domain) => {
    try {
      ...
      const subjectJson = await backendClient.get(
        `/productApi/domains/${domain}`
      );
